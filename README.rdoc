...
== Static tables
If you dont want to have an ajax table, dont specify the ajax_url. The table will
be rendered without pagination with all data that is found.
=== Example
    def index
      fancygrid_for :users do |g|
        # ...
...
on the columns name to determine what to render. You should never omit the else case. 
If this is missing, you might end up with blank columns.
=== Example
